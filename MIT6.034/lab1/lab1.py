...
SON_RULE = IF(AND('male (?y)', 'parent (?x) (?y)'),
                  THEN('son (?y) (?x)'))

DAUGHTER_RULE = IF(AND('female (?y)', 'parent (?x) (?y)'),
                  THEN('daughter (?y) (?x)'))

SIBLING_RULE = IF(AND(OR('male (?x)', 'female (?x)'),
                      OR('male (?y)', 'female (?y)'),
                      NOT('same-identity (?x) (?y)'),
                      'parent (?z) (?x)', 'parent (?z) (?y)'),
                  THEN('sibling (?x) (?y)'))

COUSIN_RULE = IF(AND('parent (?x) (?j)',
                     'parent (?y) (?k)',
                     'sibling (?x) (?y)',
                     NOT('same-identity (?j) (?k)')),
                 THEN('cousin (?j) (?k)'))
...
# family_rules.
family_rules = [SAME_MALE, SAME_FEMALE, BROTHER_RULE, SISTER_RULE, MOTHER_RULE,
                FATHER_RULE, SON_RULE, DAUGHTER_RULE, SIBLING_RULE, COUSIN_RULE,
                GRANDPARENT_RULE, GRANDCHILD_RULE]   # fill me in
...
WHAT_I_FOUND_INTERESTING = 'backchain'
